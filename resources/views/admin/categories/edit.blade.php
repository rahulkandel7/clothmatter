...
@section('main')
    <div class="py-10 px-8 w-full">
        <div class="flex justify-between py-2">
          ...
          <form action="{{route('admin.categories.update',$category->id)}}" method="post" enctype="multipart/form-data" class="py-5">
            @csrf
            @method('put')
             <div class="my-2">
                <input type="text" name="title" id="" class="w-full border-none shadow-lg rounded-md placeholder:text-gray-700 focus:ring-amber-500" placeholder="Enter Category Name" value="{{$category->title}}">
                @error('title')
